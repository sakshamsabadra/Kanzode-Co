...
import { createPartyAction, deletePartyAction } from "@/app/actions";

export default function ClientsContent({ initialClients, initialParties }: { initialClients: any[], initialParties: any[] }) {
  const [parties, setParties] = useState(initialParties);
  const [showAddParty, setShowAddParty] = useState(false);
  const [newParty, setNewParty] = useState({ name: "", address: "", email: "", phoneNumber: "" });

  const clientRows = initialClients.map((client) => ({
    ...client,
  ...
  async function handleAddParty() {
    ...
    // Note: revalidatePath will refresh the Server Component data, 
    // but since we're in a Client Component with state, we might need to sync or just let Next.js refresh.
    // In Next.js 15, revalidatePath works well with Server Components.
    // For now, I'll rely on the user refreshing or the page revalidation.
