...
export function SearchableInvoiceList({
  invoices,
  clients
}: {
  invoices: any[];
  ...
  const filtered = invoices.filter((inv) => {
    ...
    const clientMatch = clients.find((c) => c.id === inv.clientId)?.companyName?.toLowerCase().includes(term);
    const statusMatch = activeStatus ? inv.paymentStatus === activeStatus : true;
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
          ...
          },
          {
            ...
            render: (value) => (
              <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                <Link href={`/invoices/${value}`} className="px-2 py-1 text-xs font-semibold text-blue-600 hover:bg-blue-50 rounded">
                  View / Edit
                </Link>
                <button
                  onClick={async () => {
                    if (confirm("Are you sure you want to delete this invoice?")) {
                      ...
                      try {
                        await deleteInvoiceAction(value as string);
        ...
        rows={filtered}
        rowHref={(row: any) => `/invoices/${row.id}`}
      />
    </div>
  );
}
